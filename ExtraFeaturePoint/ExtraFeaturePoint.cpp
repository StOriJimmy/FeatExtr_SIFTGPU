...
// 	return 1;
	...
	//	Usage();
	//}
	////argv[1] = "G:\\==������б��������==\\Test\\����\\����+����\\126.jpg";
	////argv[2] = "G:\\==������б��������==\\Test\\����\\����+����\\126.key";
	////argv[3] = "5000";
...
#pragma omp parallel for
			...
			for (int k = 0; k < key_temp.size(); k++)
			{
				...
				fprintf(f, "%.2f %.2f %.3f %.3f\n", key_temp[k].y, key_temp[k].x, key_temp[k].s, key_temp[k].o); //�У���
...
#pragma omp parallel for
	...
	//printf("[ExtraFeaturePt] Time using: %.3lf\n", double(time_end - time_start) / CLK_TCK);
    printf("[ExtraFeaturePt] Time using: %.3lf\n", double(time_end - time_start) / CLOCKS_PER_SEC);//CLK_TCK和CLOCKS_PER_SEC等效，但是已经过时
}
